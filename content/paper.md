...
Alabor et al. [@Alabor_Stolze_2020] examined the RP debugging habits of software engineers in an observational study. They highlighted that their participants intuitively expected their debuggers step controls to work on the RP data-flow graph and were surprised that they did not. The discrepancy between the expected and actual behavior of the debugger let the subjects fall back to adding manual print statements nonetheless.
...
Possible operator log points are suggested ready-to-hand through an icon annotation within the code editor, next to the actual operator. While instrumenting their debugging hypothesis, the software engineer enables a log point by hovering the mouse pointer over its associated annotation and selects the *Add Operator Log Point* action (see Figure [2](#fig:operator-log-points)). When ready to test their hypothesis, the engineer starts the RxJS program using the built-in JavaScript debugger; no extra effort is required. Once the program is running, each enabled operator log point displays the life cycle events together with the source code that produced them. Engineers are free to enable or disable additional log points during the debugging session as they like; The life cycle event display will adapt accordingly.
